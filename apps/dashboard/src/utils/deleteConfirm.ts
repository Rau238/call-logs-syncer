...
export function buildContactDeleteConfirm(contact: ContactGroup): ConfirmDeletePayload {
  return {
    ...
    details: [
      ...
      { label: 'Active calls', value: String(contact.activeCount) },
      { label: 'Deleted from phone', value: String(contact.deletedCount) },
      { label: 'Status', value: contact.isActive ? 'Active' : 'Inactive (all calls deleted from phone)' },
